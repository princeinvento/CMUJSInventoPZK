<div class="col-span-10 py-7 px-10">
    <div class="articles-table">
        <div class="text-2xl font-bold mb-6">Authors</div>
        <div class="main-actions flex mb-7 justify-between">
        <form method="GET" action="<?php echo base_url('admin/authors'); ?>">
            <input type="text" name="search" id="search" class="bg-gray-100 py-3 px-3 rounded w-96" placeholder="Enter an Author" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
            <button type="submit" class="bg-green-900 text-white py-3 px-3 rounded">Search</button>
        </form>
        <a href="<?php echo base_url('admin/authors/add')?>" class="add-volume bg-green-900 text-white py-3 px-3 rounded">Create Author</a>
        </div>
        <table class="table w-full border">
            ...
            <?php if ($users !== false && !empty($users)): ?>
            <?php foreach($users as $user): ?>
            <tr class="text-center rounded border">
                <td class="py-10 border"><?php echo $user['complete_name']?></td>
                <td class="py-10 border"><?php echo $user['email']?></td>
                <td class="py-10 border"><?php $status_display = ($user['status'] == 1) ? 'Active' : 'Inactive'; ?>  <?php echo $status_display; ?></td>
                <td class="py-10">
                    <div class="actions justify-center flex">
                        ...
                        <a href="<?php echo base_url(); ?>admin/authors/edit/<?php echo $user['userid'];?>" class="action bg-green-900 rounded text-white p-2 mx-2">
                        ...
                        <a href="<?php echo base_url()?>author/delete/<?php echo $user['userid'];?>" class="action bg-green-900 rounded text-white p-2">
            ...
            <?php endforeach; ?>
            <?php else: ?>
            <p class="p-5">No users at the moment.</p>
            <?php endif; ?>
        </table>
    </div>
</div>